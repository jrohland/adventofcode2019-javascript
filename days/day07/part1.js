...
module.exports = async () => {
  ...
  const phaseSettings = permutator([0, 1, 2, 3, 4])
  ...
  phaseSettings.forEach(currentPhaseSettings => {
    let lastOutput = 0
    const memory = instructions.slice(0)
    currentPhaseSettings.forEach(phase => {
      const inputs = [lastOutput, phase]
      let instructionIndex = 0
      let result = false
      do {
        result = executeInstruction(memory, instructionIndex, inputs)
        if (result.output !== false && result.output !== true) lastOutput = result.output
